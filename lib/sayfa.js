...
// Nitelik içine hazır HTML yazmak için tam kaçış (var olan varlıklar da kaçırılır;
// tarayıcı niteliği okurken aynen geri çözer).
function nitelikTam(s) {
  return String(s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

function iki(n) { return ('0' + n).slice(-2); }

// Metindeki [yazı](https://adres) biçimini bağlantıya çevirir; metnin geri kalanı kaçırılır.
// Yalnızca http ve https adresleri bağlantı olur.
function baglantili(s) {
  return kacis(s).replace(/\[([^\[\]]+)\]\((https?:\/\/[^\s()"<>]+)\)/g, function (m, yazi, adres) {
    return '<a href="' + adres + '" target="_blank" rel="noopener">' + yazi + '</a>';
  });
}
...
function kapak(k) {
  var satirlar = (k.bilgiler || []).map(function (b) {
    ...
    // gorunum: 'liste' ise bütün satırlar alt alta görünür; yoksa kapakta sırayla döner.
    var liste = b.gorunum === 'liste';
    var ek = !liste && akis.length > 1 ? ' data-akis="' + nitelikTam(JSON.stringify(akis.map(baglantili))) + '"' : '';
    var icerik = liste ? akis.map(baglantili).join('<br>') : baglantili(akis[0] || '');
    return '      <li>\n'
      + '        <p class="fact-k">' + kacis(b.baslik) + '</p>\n'
      + '        <p class="fact-v' + (liste ? ' fact-liste' : '') + '"' + ek + '>' + icerik + '</p>\n'
      + '      </li>';
  });
...
/* ---------- kapaktaki araç kutuları ---------- */
// Okunabilirlik ölçer sayfada sabittir; buradaki kutular bağlantı kutularıdır.
function araclar(k) {
  return (k.araclar || []).filter(function (x) { return String(x.baglanti || '').trim(); }).map(function (x) {
    return '      <a class="probe arac-link" href="' + nitelik(String(x.baglanti).trim()) + '">\n'
      + '        <p class="probe-top">' + kacis(x.etiket) + '</p>\n'
      + '        <p class="arac-baslik">' + kacis(x.baslik) + '</p>\n'
      + '        <span class="arac-git">' + kacis(x.dugme || 'Aç') + ' <span class="ok" aria-hidden="true">→</span></span>\n'
      + '      </a>';
  }).join('\n');
}
...
function yayinSatiri(k) {
  var yazar = k.yazarlar ? kacis(k.yazarlar) + ' · ' : '';
...
function bilgiler(i) {
  return (i.bilgiler || []).map(function (b) {
    return '      <div><dt>' + kacis(b.baslik) + '</dt><dd>' + baglantili(b.deger) + '</dd></div>';
  }).join('\n');
}
...
function sayfayiUret(html, veri) {
  ...
  if (s) {
    ...
    html = blokDegistir(html, 'ARACLAR', araclar(s.kapak || {}));
...
// Node'da modül olarak, tarayıcıda (panel önizlemesi) window.Sayfa olarak kullanılır.
if (typeof module !== 'undefined' && module.exports) module.exports = api;
else if (typeof window !== 'undefined') window.Sayfa = api;
